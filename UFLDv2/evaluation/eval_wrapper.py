...
def generate_lines_col(out_col, out_col_ext, shape, names, output_path, griding_num, localization_type='abs',
                       flip_updown=False):
        ...
        with open(line_save_path, 'w') as fp:
            ...
            for i in lane_list:
                if valid[j, :, i].sum() > num_cls / 2:
                    for k in range(valid.shape[1]):
                        if valid[j, k, i]:
                            all_ind = torch.tensor(list(range(max(0, max_indices[j, k, i] - local_width),
                                                              min(out.shape[1] - 1,
                                                                  max_indices[j, k, i] + local_width) + 1)))
...
def generate_lines_col_local_curve_combine(dataset, out_col, out_col_ext, names, output_path, mode='normal',
                                           col_anchor=None):
    batch_size, num_grid_col, num_cls, num_lane = out_col.shape
    max_indices = out_col.argmax(1).cpu()
    # n, num_cls, num_lanes
...
def run_test(dataset, net, data_root, exp_name, work_dir, distributed, crop_ratio, train_width, train_height,
             batch_size=8, row_anchor=None, col_anchor=None):
    ...
    for i, data in enumerate(dist_tqdm(loader)):
        ...
        if dataset == "CULane":
            generate_lines_local(dataset, pred['loc_row'], pred['exist_row'], names, output_path, 'normal',
                                 row_anchor=row_anchor)
            generate_lines_col_local(dataset, pred['loc_col'], pred['exist_col'], names, output_path, 'normal',
                                     col_anchor=col_anchor)
        elif dataset == 'CurveLanes':
            generate_lines_local_curve_combine(dataset, pred['loc_row'], pred['exist_row'], names, output_path,
                                               row_anchor=row_anchor)
            generate_lines_col_local_curve_combine(dataset, pred['loc_col'], pred['exist_col'], names, output_path,
                                                   col_anchor=col_anchor)
...
def generate_lines_local_tta(loc_row, loc_row_left, loc_row_right, exist_row, exist_row_left, exist_row_right, names,
                             output_path, row_anchor):
    local_width = 1
                    ...
                    for cls_idx in range(num_cls):
                        ...
                        if valid[batch_idx, cls_idx, lane_idx]:
                            all_ind = torch.tensor(list(
                                range(max(0, max_indices[batch_idx, cls_idx, lane_idx] - local_width),
                                      min(num_grid - 1, max_indices[batch_idx, cls_idx, lane_idx] + local_width) + 1)))
                            out_tmp = (loc_row[batch_idx, all_ind, cls_idx, lane_idx].softmax(
                        ...
                        if valid_left[batch_idx, cls_idx, lane_idx]:
                            all_ind_left = torch.tensor(list(
                                range(max(0, max_indices_left[batch_idx, cls_idx, lane_idx] - local_width),
                                      min(num_grid - 1,
                                          max_indices_left[batch_idx, cls_idx, lane_idx] + local_width) + 1)))

                            out_tmp_left = (loc_row_left[batch_idx, all_ind_left, cls_idx, lane_idx].softmax(
                                0) * all_ind_left.float()).sum() + 0.5
                        ...
                        if valid_right[batch_idx, cls_idx, lane_idx]:
                            all_ind_right = torch.tensor(list(
                                range(max(0, max_indices_right[batch_idx, cls_idx, lane_idx] - local_width),
                                      min(num_grid - 1,
                                          max_indices_right[batch_idx, cls_idx, lane_idx] + local_width) + 1)))

                            out_tmp_right = (loc_row_right[batch_idx, all_ind_right, cls_idx, lane_idx].softmax(
                                0) * all_ind_right.float()).sum() + 0.5
...
def generate_lines_col_local_tta(loc_col, loc_col_up, loc_col_down, exist_col, exist_col_up, exist_col_down, names,
                                 output_path, col_anchor):
    local_width = 1
                    ...
                    for cls_idx in range(num_cls):
                        ...
                        if valid[batch_idx, cls_idx, lane_idx]:
                            all_ind = torch.tensor(list(
                                range(max(0, max_indices[batch_idx, cls_idx, lane_idx] - local_width),
                                      min(num_grid - 1, max_indices[batch_idx, cls_idx, lane_idx] + local_width) + 1)))
                            out_tmp = (loc_col[batch_idx, all_ind, cls_idx, lane_idx].softmax(
                                0) * all_ind.float()).sum() + 0.5
                        ...
                        if valid_up[batch_idx, cls_idx, lane_idx]:
                            all_ind_up = torch.tensor(list(
                                range(max(0, max_indices_up[batch_idx, cls_idx, lane_idx] - local_width),
                                      min(num_grid - 1,
                                          max_indices_up[batch_idx, cls_idx, lane_idx] + local_width) + 1)))
                            out_tmp_up = (loc_col_up[batch_idx, all_ind_up, cls_idx, lane_idx].softmax(
                                0) * all_ind_up.float()).sum() + 0.5
                        ...
                        if valid_down[batch_idx, cls_idx, lane_idx]:
                            all_ind_down = torch.tensor(list(
                                range(max(0, max_indices_down[batch_idx, cls_idx, lane_idx] - local_width),
                                      min(num_grid - 1,
                                          max_indices_down[batch_idx, cls_idx, lane_idx] + local_width) + 1)))
                            out_tmp_down = (loc_col_down[batch_idx, all_ind_down, cls_idx, lane_idx].softmax(
                                0) * all_ind_down.float()).sum() + 0.5
...
def run_test_tta(dataset, net, data_root, exp_name, work_dir, distributed, crop_ratio, train_width, train_height,
                 batch_size=8, row_anchor=None, col_anchor=None):
    output_path = os.path.join(work_dir, exp_name)
    if not os.path.exists(output_path) and is_main_process():
        os.mkdir(output_path)
    ...
    for i, data in enumerate(dist_tqdm(loader)):
        ...
        generate_lines_local_tta(loc_row, loc_row_left, loc_row_right, exist_row, exist_row_left, exist_row_right,
                                 names, output_path, row_anchor)
        generate_lines_col_local_tta(loc_col, loc_col_up, loc_col_down, exist_col, exist_col_up, exist_col_down, names,
                                     output_path, col_anchor)
...
def run_test_tusimple(net, data_root, work_dir, exp_name, distributed, crop_ratio, train_width, train_height,
                      batch_size=8, row_anchor=None, col_anchor=None):
    ...
    for data in dist_tqdm(loader):
        imgs, names = data
        imgs = imgs.cuda() # changed
        with torch.no_grad():
            pred = net(imgs) # changed
        for b_idx, name in enumerate(names):
            tmp_dict = {}
            tmp_dict['lanes'] = generate_tusimple_lines(pred['loc_row'][b_idx], pred['exist_row'][b_idx],
                                                        pred['loc_col'][b_idx], pred['exist_col'][b_idx],
                                                        row_anchor=row_anchor, col_anchor=col_anchor, mode='4row')
...
def eval_lane(net, cfg, ep=None, logger=None):
    ...
    if cfg.dataset == 'CurveLanes':
        if not cfg.tta:
            run_test(cfg.dataset, net, cfg.data_root, 'curvelanes_eval_tmp', cfg.test_work_dir, cfg.distributed,
                     cfg.crop_ratio, cfg.train_width, cfg.train_height, row_anchor=cfg.row_anchor,
                     col_anchor=cfg.col_anchor)
        else:
            run_test_tta(cfg.dataset, net, cfg.data_root, 'curvelanes_eval_tmp', cfg.test_work_dir, cfg.distributed,
                         cfg.crop_ratio, cfg.train_width, cfg.train_height, row_anchor=cfg.row_anchor,
                         col_anchor=cfg.col_anchor)
        ...
        if not cfg.tta:
            run_test(cfg.dataset, net, cfg.data_root, 'culane_eval_tmp', cfg.test_work_dir, cfg.distributed,
                     cfg.crop_ratio, cfg.train_width, cfg.train_height, row_anchor=cfg.row_anchor,
                     col_anchor=cfg.col_anchor)
        else:
            run_test_tta(cfg.dataset, net, cfg.data_root, 'culane_eval_tmp', cfg.test_work_dir, cfg.distributed,
                         cfg.crop_ratio, cfg.train_width, cfg.train_height, row_anchor=cfg.row_anchor,
                         col_anchor=cfg.col_anchor)
        synchronize()  # wait for all results
        if is_main_process():
        ...
        run_test_tusimple(net, cfg.data_root, cfg.test_work_dir, exp_name, cfg.distributed, cfg.crop_ratio,
                          cfg.train_width, cfg.train_height, row_anchor=cfg.row_anchor, col_anchor=cfg.col_anchor)
        synchronize()  # wait for all results
        if is_main_process():
        ...
        exp_name = 'smartrollerz_eval_tmp'
        run_test_tusimple(net, cfg.data_root, cfg.test_work_dir, exp_name, cfg.distributed, cfg.crop_ratio,
                          cfg.train_width, cfg.train_height, row_anchor=cfg.row_anchor, col_anchor=cfg.col_anchor)
        synchronize()  # wait for all results
        if is_main_process():
...
def call_curvelane_eval(data_dir, exp_name, output_path):
    ...
    print('./evaluate -s -a %s -d %s -i %s -l %s -w %s -t %s -c %s -r %s -f %s -o %s -x %s -y %s' % (
    data_dir, detect_dir, data_dir, list0, w_lane, iou, im_w, im_h, frame, out0, x_factor, y_factor))
    os.system('%s -a %s -d %s -i %s -l %s -w %s -t %s -c %s -r %s -f %s -o %s -x %s -y %s' % (
