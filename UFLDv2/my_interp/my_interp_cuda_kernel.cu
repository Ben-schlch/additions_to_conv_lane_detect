...
namespace {
    ...
    __global__ void my_interp_cuda_kernel(
            const torch::PackedTensorAccessor64<float,4,torch::RestrictPtrTraits> d_input,
            const torch::PackedTensorAccessor64<float,1,torch::RestrictPtrTraits> d_interp_loc,
            torch::PackedTensorAccessor64<float,4,torch::RestrictPtrTraits> d_output,
            int direction
        ){
...
torch::Tensor my_interp_cuda(
    ...
    my_interp_cuda_kernel<<<blocks, threads>>>(
        input.packed_accessor64<float,4,torch::RestrictPtrTraits>(),
        interp_loc.packed_accessor64<float,1,torch::RestrictPtrTraits>(),
        res.packed_accessor64<float,4,torch::RestrictPtrTraits>(),
        direction
    );
    // direction 0 for horizontal, 1 for vertical
    return res;
