...
if __name__ == "__main__":
    ...
    # training set
    names,line_txt = get_tusimple_list(args.root + "train_set/",  ['label_data_0601.json','label_data_0531.json','label_data_0313.json'])

    print(np.asarray(names).shape)
    print(line_txt)

    # generate segmentation and training list for training
    generate_segmentation_and_train_list(args.root + "train_set/", line_txt, names)

    # testing set
    names,line_txt = get_tusimple_list(args.root + "test_set/", ['test_tasks_0627.json'])
    # generate testing set for testing
    with open(os.path.join(args.root + "test_set",'test.txt'),'w') as fp:
        for name in names:
            fp.write(name + '\n')
